...
of the type for whatever criteria they think. This makes it particularly hard to write portable code among
compilers from an ABI point of view (Visual C++ vs C++ Builder, anyone?).

Some days ago I was debugging a problem with [dali-toolkit](https://github.com/dalihub/dali-toolkit)
and the enum story escalated to another level.

Let me give you some background: [dali-toolkit](https://github.com/dalihub/dali-toolkit) loads textures
using a background worker thread that is responsible for loading the image files to memory. I was looking into
...
In particular, let's look at the assembly code for the `main` function. Firstly, the gcc generated
...
Very smart! Too bad that code is wrong. I think it is easier to understand the issue if we
view the bit layout of the word held by AnimalInfo:
...
Once we understand the problem, we can solve it easily by forcing the enum to an unsigned integer:
