...
to parameterize the task and very difficult to represent task relationships.
...
and if all tests pass, we run a task responsible for updating the OTA server.
...
were discussions at the Berlin work week about whether we should stick with yaml files
...
top-down syntax, since most scenarios call for a top-down approach. Another
question is whether to put the description of task relationships inside the task
files or in a separate configuration file. We would like to represent task
...
root.yml which only contains root tasks.
