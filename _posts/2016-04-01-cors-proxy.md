...
for which it is safe to make these cross-domain requests, they can either implement
...
CORS is a mechanism that allows the web server to tell the browser that it is safe to
...
To allow the monitoring dashboard to make requests for status state on remote services
...
Before you think of using the hosted server to bypass your own requests, cors-proxy
