...
tracepoint enabled, is this mitigation on, is this debug feature live. A
normal `if (flag)` still loads that flag from memory on every hit. Jump
labels — the mechanism behind the static-key API — rewrite the machine
code instead. While the feature is off, the hot path is a `nop` (or an
unconditional `jmp`). Turning the feature on patches `nop` \<=\> `jmp`
in place. Toggling is expensive; running the common path is nearly free.

The first part of the document, that goes from section
[§1](#the-problem-jump-labels-solve) to
[§3](#how-to-use-static-keys-the-cookbook), is focused on the API usage.
If you just want to learn static keys to use them in your own code, you
can stop there.

**The rest of this document is the implementation** on **x86_64**,
pinned to kernel **7.2**. It walks the bytes the compiler emits, the
...
[`__jump_table`](https://elixir.bootlin.com/linux/v7.2/source/scripts/module.lds.S#L31).
...
penalty, not the **guaranteed memory load** in step 1. When the check
sits in a hot path that runs millions of times a second (scheduler,
networking, every `trace_*()` site), that load adds up.
...
path has no branch to the rare code — it is a `nop` (or an unconditional
`jmp`). When someone turns the feature on, the kernel walks every call
site for that key and swaps `nop` and `jmp` in place.
...
## 2 The mental model, in one diagram {#the-mental-model-in-one-diagram}

When static keys are used, the same `if` compiles to two already-built
instruction sequences. Which one sits in memory depends only on whether
the key is currently enabled:
...
Both columns come from the same source line. Which one sits in memory
depends on whether the key is enabled. “FEATURE OFF” has no branch at
all: `rare_code()` still exists in the binary, but with no `jmp`
pointing at it, normal execution cannot reach it. The `nop` is exactly
as wide as the `jmp` it might become (**2 or 5 bytes** on x86_64 — see
[§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes)), so turning
...
the `if`. That returning jump is fixed at compile time; only the
`nop`/`jmp` at the top of the site ever changes.
...
## 3 How to use static keys (the cookbook) {#how-to-use-static-keys-the-cookbook}

Toggling a key is not a cheap flag flip. It patches machine code on
every CPU. Every API choice below is about how often that happens and
who is allowed to trigger it.

### 3.1 Minimal example {#minimal-example}
...
`foo_key` is a
[`struct static_key_false`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L355):
an
[`atomic_t`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/types.h#L188)
wrapped in a type the compiler can tell apart from
[`static_key_true`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L351).
`_FALSE`, paired with
[`static_branch_unlikely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L486),
is the combination that compiles to a `nop` while the key stays at its
initial value.

### 3.2 Choosing TRUE vs FALSE and likely vs unlikely {#choosing-true-vs-false-and-likely-vs-unlikely}

Getting this pair wrong is not a correctness bug — the code still runs —
it is a silent performance foot-gun. Pick the pairing that disagrees
with steady state, and you may end up with a `jmp` sitting on the hot
path where a `nop` belonged. That mistake is invisible in any
correctness test, and it cannot be fixed by toggling the key later:
which of
[`arch_static_branch`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L35)
or
[`arch_static_branch_jump`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L45)
gets compiled in is baked in at build time.

Ask two questions up front:

1.  **What is the default at boot?** Most optional features start
    disabled →
    [`DEFINE_STATIC_KEY_FALSE`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L371).
...
2.  **Which way does *this* `if` lean in steady state?** Use
    ...
    [`static_branch_likely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L474)
    when the body is the common path.

You can mix them: a `_FALSE` key works with both `likely` and
`unlikely`, and so does a `_TRUE` key. The kernel picks a compiled-in
`nop` or `jmp` so the **default** case is the cheap one.
...
### 3.3 Boolean enable vs refcounted enable {#boolean-enable-vs-refcounted-enable}
...
both call
[`static_branch_enable()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L522)
on the same key, each wanting the feature on for its own reasons.
Whichever one finishes first and calls
[`static_branch_disable()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L523),
turns the feature off for both, even if the other owner is not done.
Refcounting exists precisely to handle such cases.
...
[`key->enabled`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L87)
counter, but each assumes a different range of values as valid: the
boolean API assumes 0 or 1, while `inc`/`dec` assumes an integer
counting value.

If some other
[`static_branch_inc()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L513)
caller has already pushed the count to 2 or higher,
[`static_key_enable()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L220)
and
[`static_key_disable()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L245)
won’t corrupt that count — but they won’t do what the caller expects,
either.
[`static_key_enable()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L220)
treats any value above zero as “already on” and returns immediately;
[`static_key_disable()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L245)
...
only a kernel warning to show that something went wrong.

### 3.4 Reading the state without taking the branch {#reading-the-state-without-taking-the-branch}

Every example so far, including the `hot_path()` from
[§3.1](#minimal-example), uses
[`static_branch_likely`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L474)/[`static_branch_unlikely`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L486)
...
branch to take.
[`static_key_enabled()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L407)
is exactly that:
...
[`arch/x86/kernel/cpu/bugs.c`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/cpu/bugs.c#L1893)
...
[`drivers/md/dm-stats.c`](https://elixir.bootlin.com/linux/v7.2/source/drivers/md/dm-stats.c#L419)
does
...
specifically to avoid re-triggering a full patch round when the feature
is already on.
...
load [§1](#the-problem-jump-labels-solve) started from.

### 3.5 Keys must be global / static storage {#keys-must-be-global-static-storage}

A static key **cannot** live on the stack or be
[`kmalloc`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/slab.h#L1051)’d. The
compiler embeds its address into
[`__jump_table`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L209)
as a link-time constant (a relative offset, [§6.3](#relationship)) —
...
[`kmalloc`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/slab.h#L1051)’d
...
### 3.6 Read-only-after-init keys {#read-only-after-init-keys}
...
toggle it”, even a bug. `DEFINE_STATIC_KEY_FALSE_RO` (and its `_TRUE_RO`
...
Placed in
[`__ro_after_init`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/cache.h#L60).
You may still
[`static_branch_enable`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L522)/[`static_branch_disable`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L523)
**during `__init`** (before
[`mark_rodata_ro()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/mm/init_64.c#L1405)).
After that:

- The key struct — including
  [`enabled`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L87)
  — is mapped read-only, so further enable/disable/inc/dec would fault
  on the atomic write.
- [`jump_label_init_ro()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L573)
  has **sealed** the key ([§10](#modules-the-trickiest-part)): cleared
  its `entries` pointer and set
  [`JUMP_TYPE_LINKED`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L196),
  so even if something could write
  [`enabled`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L87),
  [`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886)
...
life of the system, use plain `DEFINE_STATIC_KEY_*`.
...
### 3.7 Rate-limited disable (userspace-facing knobs) {#rate-limited-disable-userspace-facing-knobs}

If userspace can flip a feature rapidly, naively patching on every
toggle thrashes text and tanks performance: a single toggle is a full
IPI round to every online CPU, not just a local write. A sysctl or
socket option a user flips and unflips in a loop would otherwise turn
into a machine-wide synchronization storm, one round per flip.

The deferred versions of the \[`static_branch_inc/dec()`\] APIs are
deliberately **asymmetric**, and that asymmetry is the whole design.
[`static_branch_deferred_inc()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label_ratelimit.h#L97)
is just an alias for
[`static_branch_inc()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L513)
from [§3.3](#boolean-enable-vs-refcounted-enable), with no delay at all.
If this key is guarding something like counting or tracing, a delayed
enable would mean silently missing whatever happened during the delay.
Turning it *off* is the side that can safely wait, since a feature
staying active a little longer than strictly necessary is harmless.

[`static_branch_slow_dec_deferred()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label_ratelimit.h#L29)
...
- **Not the last reference** (the count is still above 1 after
  ...
  from an ordinary `dec` in [§3.3](#boolean-enable-vs-refcounted-enable)
...
- **This decrement would be the last reference** (the count is at 1,
  ...
  [`static_key_dec_not_one()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L253)
  detects this case up front and deliberately leaves the count untouched
  at 1, then hands off to
  [`schedule_delayed_work()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/workqueue.h#L853),
  which arms a timer for `timeout` jiffies. Only once that timer fires
  does
  [`jump_label_update_timeout()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L325)
  run the real decrement and trigger the patch-off.
...
That is what makes the coalescing work. If a fresh increment arrives
during that window, the count moves from 1 to 2 (the fast path from
[§3.3](#boolean-enable-vs-refcounted-enable)) — *before* the timer has
...
final one. By now, it genuinely isn’t: the count drops from 2 to 1, so
the “is this the transition to zero” check inside the real dec path
...
[`static_key_deferred_flush()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label_ratelimit.h#L32)
blocks until any pending deferred disable has actually run. Call it
before freeing the enclosing
[`struct static_key_false_deferred`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label_ratelimit.h#L21)
— the struct that bundles the key, the `timeout`, and the
`delayed_work`. That memory usually is about to go away, e.g. a module
being unloaded. Free it with the timer still armed, and when the timer
fires,
[`jump_label_update_timeout()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L325)
will run on a `delayed_work` that no longer exists: a use-after-free.

------------------------------------------------------------------------

## 4 Hardware background (why this is hard) {#hardware-background-why-this-is-hard}

The sections above treat `nop` and `jmp` as the two shapes of a site.
The rest of this document is about emitting those bytes and swapping
them while the kernel is running. Three hardware facts sit under that:
how a core treats instructions and memory loads, the exact encodings,
and why a live multi-core kernel cannot overwrite them with an ordinary
write.

### 4.1 What a CPU actually does with instructions {#what-a-cpu-actually-does-with-instructions}

A modern x86_64 core does not “read one instruction, execute it,
repeat”. Roughly:

1.  **Fetch** bytes from the instruction cache (I-cache / L1i).
2.  **Decode** those bytes into micro-ops (variable-length on x86 — an
...
guess still happens on every hit, and it still claims a data-cache
access and an execution port each time. Under cache pressure, or if some
other CPU writes that flag and bounces its cache line, the “cheap”
branch stops being cheap at all.

An unconditional `nop` or `jmp` has no flag to load and no condition to
evaluate. The “off” path is empty work for the frontend — decode a
`nop`, move on — with no cache line to bounce and nothing for the branch
predictor to weigh in on.

### 4.2 x86 instruction encoding: JMP and NOP {#x86-instruction-encoding-jmp-and-nop}

A patch can swap `nop` and `jmp` only if they occupy the same number of
bytes. x86 is a variable-length ISA; the encodings jump labels care
about:
...
Constants live in
[`arch/x86/include/asm/text-patching.h`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/text-patching.h)
(`JMP8_INSN_*`, `JMP32_INSN_*`, `INT3_INSN_*`) and
[`arch/x86/include/asm/nops.h`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/nops.h)
([`BYTES_NOP5`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/nops.h#L60),
...
[`text_gen_insn()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/text-patching.h#L123)
/
[`__text_gen_insn()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/text-patching.h#L93)
compute.
...
### 4.3 Why you cannot just `memcpy` over live code on SMP {#why-you-cannot-just-memcpy-over-live-code-on-smp}
...
1.  It is mapped **read-only** after boot
    ([`STRICT_KERNEL_RWX`](https://elixir.bootlin.com/linux/v7.2/source/arch/Kconfig#L1581)),
...
Point 2 is the dangerous one. Picture two CPUs, A and B, where A is
patching a 5-byte instruction that B keeps calling in a loop. The store
A makes is not one atomic operation — the CPU issues it as however many
bus-width writes it takes to cover 5 bytes, and each of those writes
becomes visible to the rest of the system separately. The fetch B makes
can land in the middle of that sequence, seeing some bytes from before
the write and some from after:
...
([§9](#x86-text-patching-the-gory-details)), and jump labels are only
...
### 4.4 Writing read-only kernel text: [`text_poke()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2668) {#writing-read-only-kernel-text-text-poke}
...
Physical RAM does not know or care how it is mapped; the same page of
memory can be reached through more than one virtual address at once,
each with its own permissions. `.text` normally has exactly one mapping,
visible to every CPU, always read-only and executable. That single
mapping is what lets any core fetch and run it at any moment.
...
[`virt_to_page()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/page.h#L62)
for core kernel text, or
[`vmalloc_to_page()`](https://elixir.bootlin.com/linux/v7.2/source/mm/vmalloc.c#L821)
...
[`text_poke_mm`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2515)
(allocated once, at boot, by
[`poking_init()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/mm/init.c#L819)).
That entry is marked writable, and deliberately *not* global: it carries
no
[`_PAGE_GLOBAL`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/pgtable_types.h#L59)
bit.
...
[`flush_tlb_mm_range()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/mm/tlb.c#L1428)
...
via
[`use_temporary_mm(text_poke_mm)`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/mm/tlb.c#L992),
...
[`text_poke_mm_addr`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2516)
...
So
[`use_temporary_mm()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/mm/tlb.c#L992)
explicitly calls
[`hw_breakpoint_disable()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/debugreg.h#L109)
right after the switch, and its counterpart,
[`hw_breakpoint_restore()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/hw_breakpoint.c#L484),
...
window. The window is short enough that the kernel accepts the
suppression.
...
For a real
[`text_poke()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2668)
call, `func` is
[`text_poke_memcpy()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2528)
— the small wrapper the caller handed in as the `func` argument (the
`_set()` variant passes the memset-flavored
[`text_poke_memset()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2535)
instead):
...
than a real call to
[`memcpy()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/lib/memcpy_64.S#L43).
The hardware sidebar below answers both together.

Fifth, the temporary mapping is dismantled in the reverse order it was
built:
[`pte_clear()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/pgtable.h#L88)
removes the page-table entry,
[`unuse_temporary_mm()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/mm/tlb.c#L1027)
...
[`_set`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2744)/[`_copy`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2727)
...
[`BUG()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/bug.h#L114):
a silent failure here would leave the CPU executing bytes the patcher
...
is serialized by
[`text_mutex`](https://elixir.bootlin.com/linux/v7.2/source/kernel/extable.c#L27),
...
>   [`_PAGE_USER`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/pgtable_types.h#L53)
...
>
> [`text_poke_mm_addr`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2516)
> falls right in the gap between those two rules. It is **not** a
> userspace mapping:
> [`poking_init()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/mm/init.c#L819)
> sets it to
> [`TASK_UNMAPPED_BASE`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/processor.h#L684),
> plus a KASLR-style random offset — the same range where calls to
> [`mmap()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/sys_x86_64.c#L82)
> made by an ordinary process would land — because
> [`text_poke_mm`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2515)
> is built with
> [`mm_alloc()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/fork.c#L1166),
> the ordinary allocator for a process address space, and a freshly
> allocated `mm` has empty space there. The page-table entry actually
> built at that address is an ordinary kernel-only mapping, with
> [`_PAGE_USER`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/pgtable_types.h#L53)
...
> [`lass_stac()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/smap.h#L70)/[`lass_clac()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/smap.h#L65)
> rather than plain `STAC`/`CLAC`. They emit the same underlying
> instructions, just gated on
> [`X86_FEATURE_LASS`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/cpufeatures.h#L317)
> instead of
> [`X86_FEATURE_SMAP`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/cpufeatures.h#L253)
...
> ([`copy_from_user()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/uaccess.h#L218)
...
> [`objtool`](https://elixir.bootlin.com/linux/v7.2/source/tools/objtool)
> ([§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes) covers it in
...
> transitively reach
> [`schedule()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/sched/core.c#L7316)
...
> [`__inline_memcpy()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/string.h#L11)/[`__inline_memset()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/string.h#L21)
...
## 5 What the compiler emits (x86_64) {#what-the-compiler-emits-x86-64}

Each site is a `nop` or a `jmp` of equal width, plus a sidecar entry in
[`__jump_table`](https://elixir.bootlin.com/linux/v7.2/source/scripts/module.lds.S#L31).
Which shape is compiled in depends on the TRUE/FALSE type of the key and
the likely/unlikely hint at the site; two arch helpers emit the bytes.

### 5.1 TRUE/FALSE keys and likely/unlikely sites {#two-polarities-key-default-branch-hint}

The compiler has to emit a real instruction at each site before
[`jump_label_init()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L525)
runs, so it cannot read the live
[`enabled`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L87)
count. It determines the code generated at compile time:

1.  **Key initial value** — `DEFINE_STATIC_KEY_TRUE` vs `_FALSE`.
2.  **Call-site hint** —
    [`static_branch_likely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L474)
    vs
    [`static_branch_unlikely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L486).

When those agree, the compiled-in instruction is `nop`. When they
disagree, it is `jmp`. Get them out of sync with the steady state of the
key — the mistake [§3.2](#choosing-true-vs-false-and-likely-vs-unlikely)
warns about — and the hot path starts as a `jmp` and stays that way
until someone toggles the key.

The comment at
[`include/linux/jump_label.h`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L31)
draws the matrix:
...
The runtime patcher uses the same rule, with the live count in place of
the declared default.
[`jump_label_type()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L455)
is that rule;
[`jump_label_init_type()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L603)
is identical except it reads the compile-time `type` bit of the key.
`branch` is 1 for `likely`, 0 for `unlikely`. At compile time the same
XOR is `type ^ branch`. The function below is the live half:
...
### 5.2 How the macros pick the asm {#how-the-macros-pick-the-asm}

[`static_branch_likely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L474)
and
[`static_branch_unlikely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L486)
never touch a patchable site directly. Underneath them sit two
...
- [`arch_static_branch`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L35)
  ...
  [§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes), a `jmp` that
...
Both helpers report which path execution took through the patched
instruction: a `nop` falls through and the call returns `false`; a `jmp`
goes to the out-of-line body and the call returns `true`.
[§5.3](#the-two-asm-helpers) shows the `asm goto` behind that.

The return value is which instruction is at the site *right now*.
Whether that means “the if-body runs” is the `!` fix-up after the table
below.

Four (key type, hint) combinations collapse onto those two helpers.
**Which helper to call** follows the TRUE/FALSE type of the key.
**Whether to negate the result** follows likely vs unlikely:
...
[`arch_static_branch`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L35)
is the helper for the two cells of the 2×2 that compile to `nop`;
[`arch_static_branch_jump`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L45)
is the helper for the two that compile to `jmp`.

The lone `!` in front of the two `likely` calls makes the returned
boolean mean “does the if-body run?” for either helper. The helpers
themselves report fall-through vs jump, not “is the key enabled.”

For a TRUE key read with `likely`, the table calls the nop-default
helper. A `nop` falls through, so
[`arch_static_branch`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L35)
returns `false` even though the if-body runs: it is reporting “no jump
was taken.” The `!` turns that into “yes, the if-body runs.”

The `true`/`false` argument is also stored in the jump-table entry for
this site ([§5.4](#the-jump-table-entry-sidecar-metadata)). The runtime
patcher rereads it whenever the key toggles.

[`__builtin_types_compatible_p`](https://gcc.gnu.org/onlinedocs/gcc/Other-Builtins.html)
distinguishes
[`struct static_key_true`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L351)
vs
[`struct static_key_false`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L355)
at compile time; anything else calls
[`____wrong_branch_error()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L405)
(unresolved symbol → link error).

### 5.3 The two asm helpers {#the-two-asm-helpers}

[§5.2](#how-the-macros-pick-the-asm) treated
[`arch_static_branch()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L35)/[`arch_static_branch_jump()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L45)
as black boxes that return `false` or `true`; this is what is actually
inside them, from
[`arch/x86/include/asm/jump_label.h`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h):
...
These are two separate C functions, rather than one shared function that
takes an extra argument telling it whether to build a `nop`-shaped site
...
the macros in [§5.2](#how-the-macros-pick-the-asm) decide *which one to
...
[§5.4](#the-jump-table-entry-sidecar-metadata), explained once:
...
- **Numeric local labels**, like the bare `1:` seen below, are
  ...
  over (as
  [`JUMP_TABLE_ENTRY`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L15)
...
- **`.` (a lone dot)** is the *current location counter* of GAS: the
  ...
  [§5.4](#the-jump-table-entry-sidecar-metadata) explains why the
...
- `1:` defines the local label that the jump-table entry for this site
  ...
  [§5.4](#the-jump-table-entry-sidecar-metadata)).
- `jmp %l[l_yes]` is a plain, unconditional x86 jump — no condition
  ...
  [§5.1](#two-polarities-key-default-branch-hint) keeps referring to: it
  exists, unconditionally, in the object file the moment this
  translation unit is assembled, before anything has been patched.
- [`JUMP_TABLE_ENTRY`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L15)
  (`"%c0 + %c1"`, `"%l[l_yes]"`) expands in place right there, splicing
  a jump-table entry describing *this exact* `1:`/`jmp` pair into the
  assembly stream. [§5.4](#the-jump-table-entry-sidecar-metadata) covers
  what that entry stores and why; the mechanical detail here is the pair
...
[`ARCH_STATIC_BRANCH_ASM`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L26)
macro:
...
[`HAVE_JUMP_LABEL_HACK`](https://elixir.bootlin.com/linux/v7.2/source/arch/Kconfig#L1399)
([§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes) explains what
...
- **With the hack** (the normal case on x86_64): line for line, this is
  ...
  ([§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes) and
  [§5.6](#assembly-level-picture) explain that signal in full).
- **Without the hack**: `1: .byte 0x0f,0x1f,0x44,0x00,0x00`. `.byte`
  ...
  [§5.6](#assembly-level-picture). Those particular bytes happen to
...
[§5.4](#the-jump-table-entry-sidecar-metadata) needs the actual numeric
...
### 5.4 The jump table entry (sidecar metadata) {#the-jump-table-entry-sidecar-metadata}

Each site also emits a
[`struct jump_entry`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L111)
into
[`__jump_table`](https://elixir.bootlin.com/linux/v7.2/source/include/asm-generic/vmlinux.lds.h#L436).
The macro writes no executable code:
...
[§5.3](#the-two-asm-helpers) introduced — none of it is a CPU
...
- `.pushsection __jump_table, "aw"` switches which section the assembler
  ...
  [§5.3](#the-two-asm-helpers) mentioned, now with its second argument
...
- `_ASM_ALIGN` expands to a plain `.balign 8` on x86_64 (`.balign 4` on
  ...
  is worth avoiding.
- [`ANNOTATE_DATA_SPECIAL`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/annotate.h#L112)
...
- `.long 1b - .` is the first real data field. `.long`, one of the
  directives [§5.3](#the-two-asm-helpers) named, emits exactly 4 bytes
  holding the value of whatever expression follows it — here, `1b - .`.
  Both symbols are the ones [§5.3](#the-two-asm-helpers) introduced:
...
- `_ASM_PTR " " key " - ."` is the third field, with one difference:
  ...
  [§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes) explains the
  ...
  [`jump_entry_is_branch()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L153)
  reads back later.
- `.popsection` undoes the `.pushsection` from the first line, restoring
  ...
  macro was expanded, exactly as in [§5.3](#the-two-asm-helpers).
...
in the same function, so a 32-bit displacement cannot help but reach.
The
[`static_key`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L86)
itself carries no such guarantee. As the comment on
[`struct jump_entry`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L111)
...
a 32-bit offset could not promise.
...
[§6.2](#struct-jump-entry-relative-form)) are computed the same way,
each from *its own* field address rather than that of the struct:
[`jump_entry_target()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L122)
is `&entry->target + entry->target`, and
[`jump_entry_key()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L127)
...
([§6.2](#struct-jump-entry-relative-form)) with zero patching at boot.
...
### 5.5 [`HAVE_JUMP_LABEL_HACK`](https://elixir.bootlin.com/linux/v7.2/source/arch/Kconfig#L1399): why sites are 2 *or* 5 bytes {#have-jump-label-hack-why-sites-are-2-or-5-bytes}

[§5.3](#the-two-asm-helpers) showed the nop-default site of
[`arch_static_branch`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L35)
...
is settled long before the kernel ever boots: get the compiler to emit a
real `jmp` (so it can pick whichever encoding is actually shortest for
that call site), then have a separate build step convert that `jmp` into
a NOP of the *same* size, after compilation but before the kernel image
is final.

That trick is gated by
[`HAVE_JUMP_LABEL_HACK`](https://elixir.bootlin.com/linux/v7.2/source/arch/Kconfig#L1399).
[`arch/x86/Kconfig`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/Kconfig)
turns it on for any build with objtool available
(`select HAVE_JUMP_LABEL_HACK if HAVE_OBJTOOL`) — which, in practice,
means every modern x86_64 build. The
[`ARCH_STATIC_BRANCH_ASM`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L26)
`#ifdef` from [§5.3](#the-two-asm-helpers) is the compile-time switch.
...
1.  The compiler emits a **real `jmp`** to `l_yes`, exactly as it would
    ...
    [§5.3](#the-two-asm-helpers) and
    [§5.4](#the-jump-table-entry-sidecar-metadata) used for jump-table
...
3.  **objtool**
    ...
    **same-sized NOP**, clearing the relocation that would otherwise
    ...
    [`jump_entry_set_init()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L163)
...
The catch is that
[`struct jump_entry`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L111)
([§5.4](#the-jump-table-entry-sidecar-metadata)) never records which of
...
machinery
([`arch_jump_entry_size()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/jump_label.c#L20),
[`insn_decode_kernel()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/insn.h#L172),
[`__jump_label_patch()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/jump_label.c#L36))
is exactly what [§7](#size-of-the-patchable-site-on-x86-runtime) walks
...
of this. They use the `#else` branch of
[`ARCH_STATIC_BRANCH_ASM`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L26)
shown in [§5.3](#the-two-asm-helpers): a hand-written `.byte BYTES_NOP5`
that always emits a fixed 5-byte NOP, no matter how close the target
label actually is, and patching such a site later always installs a
5-byte `JMP32` to match. The result is still correct — the branch still
works exactly the same way — it just never gets the chance to use the
shorter 2-byte encoding, so every nop-default site costs 3 extra bytes
of I-cache footprint compared to a hack-enabled build.

That whole `objtool` rewrite (the numbered steps above) only applies to
[`arch_static_branch`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L35),
the nop-default helper from [§5.3](#the-two-asm-helpers).
[`arch_static_branch_jump`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L45),
the jmp-default helper, builds its `asm goto` directly (also shown in
[§5.3](#the-two-asm-helpers)) instead of going through
[`ARCH_STATIC_BRANCH_ASM`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L26),
and its
[`JUMP_TABLE_ENTRY`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L15)
...
[`struct static_key`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L86)
pointer ([§5.4](#the-jump-table-entry-sidecar-metadata)) — does double
...
repurposed as what
[`jump_entry_is_init()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L158)
reads back as the `__init`-text flag ([§8.1](#boot-jump-label-init)).
...
inside
[`jump_label_init()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L525)
([§8.1](#boot-jump-label-init)), and read back by
[`jump_entry_is_init()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L158)
— the same bit, doing unrelated jobs on either side of the build/boot
line shown above.

### 5.6 Assembly-level picture {#assembly-level-picture}

[§5.3](#the-two-asm-helpers) gave the two C helpers,
[§5.4](#the-jump-table-entry-sidecar-metadata) gave the jump-table entry
they emit, and [§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes)
explained why the bytes at the patchable site itself are not fixed. Put
...
[§5.3](#the-two-asm-helpers) (`nopl 0x0(%rax,%rax,1)` — a multi-byte
...
`XCHG AX, AX`). Combining them does not change what the CPU actually
...
finished
[§5.4](#the-jump-table-entry-sidecar-metadata)–[§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes),
...
- `+2` is not a fourth polarity option alongside `key`/`branch` — it is
  ...
  [§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes), living in
  ...
  ([§8.1](#boot-jump-label-init)) once boot begins.

x86 also defines
[`HAVE_JUMP_LABEL_BATCH`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L5),
which is what makes the batched, IPI-amortized patch path in
[§8](#life-of-a-static-key-boot-enable-disable)–[§9](#x86-text-patching-the-gory-details)
...
Two structs and one linker-collected table. The rest of the document
keeps pointing at these.

### 6.1 [`struct static_key`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L86) {#struct-static-key}
...
**[`enabled`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L87)**
is the live state [§3](#how-to-use-static-keys-the-cookbook) and
[§5.1](#two-polarities-key-default-branch-hint) keep referring to: an
atomic refcount, `0` meaning off and any positive value meaning on (this
is what lets
[`static_branch_inc()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L513)/[`_dec()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L514)
in [§3.3](#boolean-enable-vs-refcounted-enable) stack multiple owners on
one key). It can also transiently hold **`-1`**, which means “the first
[`static_key_slow_inc()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L186)
on this key is in progress right now, patching the instruction stream”
([§8.2](#enabling-static-key-enable-static-branch-enable) covers that
window in detail). While that is happening, any other reader calling
[`static_key_count()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L104)
...
[`struct jump_entry`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L111)
and
[`struct static_key_mod`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L613)
...
| Bit | Macro | Meaning |
|----|----|----|
| 0 | [`JUMP_TYPE_TRUE`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L195) | compile-time initial value of the key was `true` — this is the same `type` bit the `type ^ branch` formula from [§5.1](#two-polarities-key-default-branch-hint) uses |
...
[§5.4](#the-jump-table-entry-sidecar-metadata)/[§6.2](#struct-jump-entry-relative-form)
...
[`__jump_table`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L209)
([§6.3](#relationship)) — `entries` just points at the start of that
...
([§6.4](#linker-section)). There is no way to splice the entries of a
...
[`kernel/jump_label.c`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c)
has to know any of this: accessors like
[`static_key_entries()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L409),
[`static_key_type()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L415),
[`static_key_linked()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L420),
and
[`static_key_set_entries()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L444)
...
[§5.2](#how-the-macros-pick-the-asm), used by
[`__builtin_types_compatible_p`](https://gcc.gnu.org/onlinedocs/gcc/Other-Builtins.html),
are trivial by design — each is nothing but a
[`struct static_key`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L86)
...
exactly what the
[`static_branch_likely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L474)/[`static_branch_unlikely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L486)
macros in [§5.2](#how-the-macros-pick-the-asm) rely on to pick the right
arch helper.

### 6.2 [`struct jump_entry`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L111) (relative form) {#struct-jump-entry-relative-form}

This is the struct [§5.4](#the-jump-table-entry-sidecar-metadata) has
...
[`HAVE_ARCH_JUMP_LABEL_RELATIVE`](https://elixir.bootlin.com/linux/v7.2/source/arch/Kconfig#L509)
...
[§5.4](#the-jump-table-entry-sidecar-metadata) walked through in detail
...
fallback itself (in
[`jump_entry_code()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L117)
...
2.  **KASLR immunity**, which
    [§5.4](#the-jump-table-entry-sidecar-metadata) already derived in
    ...
    ([§6.1](#struct-static-key)). The distance between two
...
[§6.1](#struct-static-key) used on the pointer field of `static_key`
itself, applied here to a different pointer for a different purpose —
carry two more pieces of information:

| Bit | Meaning |
|----|----|
| 0 | [`jump_entry_is_branch`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L153): the `branch` hint from [§5.1](#two-polarities-key-default-branch-hint)/[§5.2](#how-the-macros-pick-the-asm) and the `"%c0 + %c1"` from [§5.4](#the-jump-table-entry-sidecar-metadata) — `1` if the call site used `likely()`, `0` for `unlikely()` |
...
[§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes) followed
...
### 6.3 Relationship {#relationship}

[§6.1](#struct-static-key) and [§6.2](#struct-jump-entry-relative-form)
...
common, non-module-linked case from [§6.1](#struct-static-key)):
...
[`jump_label_sort_entries()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L80)
orders the whole
[`__jump_table`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L209)
...
([§5.4](#the-jump-table-entry-sidecar-metadata)), `key` is
...
directly. That is what the function does. It masks off the two flag bits
([§6.2](#struct-jump-entry-relative-form)) to recover the offset, then
adds its own field address back in:
...
[`key->entries`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L97)
...
machinery from [§9](#x86-text-patching-the-gory-details) needs that
...
([§5.4](#the-jump-table-entry-sidecar-metadata)): they encode “distance
...
[`jump_label_swap()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L63)
...
[`jump_entry`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L111)
([§6.2](#struct-jump-entry-relative-form)) need none of this: an
absolute address doesn’t care which slot it happens to sit in, so a
plain byte swap already works.

### 6.4 Linker section {#linker-section}

Every translation unit that expands
[`JUMP_TABLE_ENTRY`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L15)
([§5.4](#the-jump-table-entry-sidecar-metadata)) contributes its own
...
[§6.3](#relationship) assumes exists. That something is the main linker
...
[`jump_label_init()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L525)
find the bounds of the array at boot ([§8.1](#boot-jump-label-init)) —
...
[`jump_entry_key()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L127)
in [§6.3](#relationship) needed no count either.
...
the two fields
[`struct module`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/module.h#L397)
reserves for exactly this —
[`jump_entries`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/module.h#L511)
(a pointer to the start of the array owned by that module) and
[`num_jump_entries`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/module.h#L512)
...
wrapped in a
[`struct static_key_mod`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L613)
node ([§6.1](#struct-static-key)) whenever a key is shared between a
module and `vmlinux`, or between two modules.

------------------------------------------------------------------------

## 7 Size of the patchable site on x86 (runtime) {#size-of-the-patchable-site-on-x86-runtime}

[§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes) showed that a
...
in
[`struct jump_entry`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L111)
([§6.2](#struct-jump-entry-relative-form)) records which one it ended up
...
[`jump_entry_code(entry)`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L117)
from [§5.4](#the-jump-table-entry-sidecar-metadata) already knows how to
...
from [§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes), or a
...
shapes [§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes)
...
[`JUMP_LABEL_NOP_SIZE`](https://elixir.bootlin.com/linux/v7.2/source/arch/arm64/include/asm/jump_label.h#L17)
...
[§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes) means the width
of a site is a fact about *that specific call site*, not a constant true
of the whole kernel. The architecture-independent
[`jump_entry_size()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L171)
...
[`JMP8_INSN_OPCODE`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/text-patching.h#L60)
(`0xEB`) and
[`JMP32_INSN_OPCODE`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/text-patching.h#L57)
(`0xE9`) are the same two jump encodings named in the `rel8`/`rel32`
explanation from
[§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes), just given
their real opcode values here.
[`text_gen_insn()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/text-patching.h#L123)
builds the actual `jmp` bytes for whichever opcode it is given,
computing the displacement itself as `dest - (addr + size)` — precisely
the “distance from the address of the *next* instruction” convention
[§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes) described for
`rel8`/`rel32` encodings, just computed here at patch time instead of by
the assembler at build time.
[`x86_nops`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L91)
is a small lookup table of ready-made no-op byte sequences, one per
length, so
[`x86_nops[size]`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L91)
...
A
[`memcmp()`](https://elixir.bootlin.com/linux/v7.2/source/lib/string.c#L655)
...
[`pr_crit`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/printk.h#L543)
and calls
[`BUG()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/bug.h#L114),
...
## 8 Life of a static key: boot, enable, disable {#life-of-a-static-key-boot-enable-disable}
...
- **`0`** — off. Every site for this key holds whichever instruction
  ...
  ([§5.1](#two-polarities-key-default-branch-hint)).
...
reads as briefly stale “on” instead (see [§8.3](#disabling)), which is
harmless.

Above value `1`, a second, independent ladder exists purely for
refcounting —
[`static_branch_inc()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L513)/[`static_branch_dec()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L514)
([§3.3](#boolean-enable-vs-refcounted-enable)) climb and descend it
...
That is why the refcounted API in
[§3.3](#boolean-enable-vs-refcounted-enable) exists: many callers can
share a key without each one paying for a text-patch round-trip — the
cost of patching is paid exactly once, by whichever caller happens to be
the first to enable it or the last to disable it.

### 8.1 Boot: [`jump_label_init()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L525) {#boot-jump-label-init}
...
[`__jump_table`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L209)
from every translation unit into one array ([§6.4](#linker-section)),
...
([§6.1](#struct-static-key)) is nothing useful. In other words: the raw
table of call sites exists, but nothing yet knows *which* sites belong
to *which* key, so
[`static_branch_enable()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L522)
or
[`static_key_slow_inc()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L186)
([§3.3](#boolean-enable-vs-refcounted-enable),
[§8.2](#enabling-static-key-enable-static-branch-enable)) would have
nothing to walk if called this early.

[`arch/x86/xen/multicalls.c`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/xen/multicalls.c)
declares
[`static struct static_key mc_debug __ro_after_init;`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/xen/multicalls.c#L60)
and registers
[`xen_mc_debug`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/xen/multicalls.c#L78)
as an
[`early_param()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/init.h#L354)
whose
[`xen_parse_mc_debug()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/xen/multicalls.c#L71)
callback calls
[`static_key_slow_inc(&mc_debug)`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L186)
directly, synchronously, while
[`parse_early_param()`](https://elixir.bootlin.com/linux/v7.2/source/init/main.c#L743)
...
boolean,
[`static_key_initialized`](https://elixir.bootlin.com/linux/v7.2/source/init/main.c#L174)
...
[`start_kernel()`](https://elixir.bootlin.com/linux/v7.2/source/init/main.c#L972)
in
[`init/main.c`](https://elixir.bootlin.com/linux/v7.2/source/init/main.c),
strictly before
[`parse_early_param()`](https://elixir.bootlin.com/linux/v7.2/source/init/main.c#L743)
...
sites for this key as a linear scan” from [§6.3](#relationship) and the
“batch must stay address-ordered” requirement from
[§9.2](#batching-api-used-by-jump-labels) both work later.

The loop that follows does three things per entry, in this order,
trusting that the table is already sorted:

1.  Optionally run
    [`arch_jump_label_transform_static`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L402)
    for NOP sites — **a no-op on x86**, since `objtool` and the compiler
    have already left the right bytes in place
    ([§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes)); other
...
of each key ([§5.1](#two-polarities-key-default-branch-hint)). What this
...
[`mark_readonly()`](https://elixir.bootlin.com/linux/v7.2/source/init/main.c#L1511).
It walks
[`__jump_table`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L209)
a second time, but this pass skips almost everything — it only acts on
keys that
[`is_kernel_ro_after_init()`](https://elixir.bootlin.com/linux/v7.2/source/include/asm-generic/sections.h#L183)
...
[`static_key_seal()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L567)
keeps only the
[`JUMP_TYPE_TRUE`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L195)
bit of the current
[`key->type`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L96)
and throws the rest of the word away, replacing it with
[`JUMP_TYPE_LINKED`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L196)
plus that one preserved bit. Recall from [§6.1](#struct-static-key) that
...
from [§10](#modules-the-trickiest-part). Sealing collapses it down to
...
bit; `L` is
[`JUMP_TYPE_LINKED`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L196).
[`static_key_sealed()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L562)
...
The payoff shows up in [§10](#modules-the-trickiest-part). When a module
loaded afterward references a sealed key,
[`jump_label_add_module()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L690)
skips allocating and chaining a
[`struct static_key_mod`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L613)
for it — the bookkeeping [§10](#modules-the-trickiest-part) otherwise
...
### 8.2 Enabling: [`static_key_enable()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L220) / [`static_branch_enable()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L522) {#enabling-static-key-enable-static-branch-enable}

[`jump_label_init()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L525)
([§8.1](#boot-jump-label-init)) only builds bookkeeping — it never flips
a key. Every site in the kernel image boots running whatever `nop`/`jmp`
the compiler and `objtool` baked in
([§5.1](#two-polarities-key-default-branch-hint),
[§5](#what-the-compiler-emits-x86-64)), on or off, and stays that way
...
([§3.4](#reading-the-state-without-taking-the-branch)): the first time a
user asks the device-mapper stats ioctl to start recording per-region
I/O counters, it flips
[`stats_enabled`](https://elixir.bootlin.com/linux/v7.2/source/drivers/md/dm.c#L76),
...
([§3.4](#reading-the-state-without-taking-the-branch)). But the first
time through, this
[`static_branch_enable(&stats_enabled)`](https://elixir.bootlin.com/linux/v7.2/source/drivers/md/dm-stats.c#L420)
...
[`static_key_enable_cpuslocked()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L197)
...
1.  It first checks whether the key is already enabled, and bails out if
    so.
2.  [`jump_label_mutex`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L23)
...
4.  [`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886)
    does the actual work: it patches every site for this key
    ([§8.5](#jump-label-update-jump-label-update)).
5.  Finally, it stores `1`.

[`static_key_enable()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L220)
wraps this in
[`cpus_read_lock()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/cpu.c#L488)
so CPUs cannot come online mid-patch.

The refcounted path,
[`static_key_slow_inc_cpuslocked()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L151),
is not just this same function reused for `inc`/`dec`
([§3.3](#boolean-enable-vs-refcounted-enable)). With multiple
...
[`jump_label_mutex`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L23)
...
[`static_key_fast_inc_not_disabled()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L127)
is what makes “further incs” in the table from
[§3.3](#boolean-enable-vs-refcounted-enable) cost nothing more than an
...
### 8.3 Disabling {#disabling}

[§8.2](#enabling-static-key-enable-static-branch-enable) walked through
...
[`static_key_fast_inc_not_disabled()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L127)
from [§8.2](#enabling-static-key-enable-static-branch-enable) on the
...
[`static_key_disable_cpuslocked()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L228)
is the mirror, in the boolean API, of the enable path from
[§8.2](#enabling-static-key-enable-static-branch-enable), but it skips
...
[§3.3](#boolean-enable-vs-refcounted-enable)) before doing anything
...
[§8.2](#enabling-static-key-enable-static-branch-enable)). Disabling has
...
[§3.4](#reading-the-state-without-taking-the-branch) already covers why
these state reads only ever need to be eventually correct, never
instruction-exact.

The refcounted side runs through a different function,
[`__static_key_slow_dec_cpuslocked()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L292),
which only reaches
[`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886)
on the one decrement that actually drives the count to `0`
([`atomic_dec_and_test()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/atomic/atomic-instrumented.h#L1380)
...
### 8.4 Deferred / rate-limited dec {#deferred-rate-limited-dec}

This is the internal counterpart to the rate-limited disable in
[§3.7](#rate-limited-disable-userspace-facing-knobs), seen here from the
function-by-function angle of this section rather than the call-site
angle of the cookbook.
[§3.7](#rate-limited-disable-userspace-facing-knobs) already covers
...
[`__static_key_slow_dec_deferred()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L346)
opens with the exact same
[`static_key_dec_not_one()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L253)
check [§8.3](#disabling) just introduced: if this decrement would not
...
key. Where the
[`__static_key_slow_dec_cpuslocked()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L292)
from [§8.3](#disabling) reaches straight for
[`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886)
...
[`jump_label_update_timeout()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L325)
runs the ordinary, undeferred decrement path from [§8.3](#disabling). If
...
### 8.5 [`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886) → [`__jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L503) {#jump-label-update-jump-label-update}

Every path in [§8.2](#enabling-static-key-enable-static-branch-enable)
through [§8.4](#deferred-rate-limited-dec) eventually funnels into this
...
The first branch is the module case from [§6.1](#struct-static-key): if
bit 1 of
[`key->type`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L96)
is set
([`LINKED`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L196)),
...
[§10](#modules-the-trickiest-part) covers
[`__jump_label_mod_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L665)
in full. Otherwise,
[`static_key_entries(key)`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L409)
recovers the pointer [§8.1](#boot-jump-label-init) stored during boot —
...
[`jump_entry_key(entry)`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L127)
`== key` — only works because of the sort from
[§8.1](#boot-jump-label-init): every site belonging to this key is
...
[`jump_label_type(entry)`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L455)
computes `enabled ^ branch` — the exact static/dynamic formula from the
table in [§5.1](#two-polarities-key-default-branch-hint), but now
...
[`JUMP_LABEL_NOP`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L186)
or a
[`JUMP_LABEL_JMP`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L187).

Before acting on that answer,
[`jump_label_can_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L465)
...
[`jump_entry_is_init`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L158)
flag from [§8.1](#boot-jump-label-init) was recorded to detect), and one
that
[`kernel_text_address()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/extable.c#L94)
...
[`arch_jump_label_transform_queue()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/jump_label.c#L123)
builds up a batch, and
[`arch_jump_label_transform_apply()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/jump_label.c#L143)
...
([§9.2](#batching-api-used-by-jump-labels)) instead of paying for one
round per site.
...
[`smp_text_poke_batch_add()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L3202).
...
[`arch_jump_label_transform()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/jump_label.c#L117)
directly instead.
...
## 9 x86 text patching: the gory details {#x86-text-patching-the-gory-details}

This is where the torn-write argument from
[§4.3](#why-you-cannot-just-memcpy-over-live-code-on-smp) turns into
working code.

### 9.1 Early boot vs live SMP {#early-boot-vs-live-smp}

Very early in boot, only the boot CPU is running and `.text` is still
writable.
[`__jump_label_transform()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/jump_label.c#L83),
...
[§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish) — with nobody
else around to race the write,
[`text_poke_early()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2489)
can just do a plain IRQ-disabled + `memcpy()` +
[`sync_core()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/sync_core.h#L58)
and be done. `.text` also happens to still be writable at this point, so
the alias trick from [§4.4](#writing-read-only-kernel-text-text-poke)
isn’t needed either — but that is a bonus the check gets for free, not
something it verifies directly:
[`smp_init()`](https://elixir.bootlin.com/linux/v7.2/source/init/main.c#L1650)
...
flag from [§8.1](#boot-jump-label-init)
([`jump_entry_is_init()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L158)).
It is a separate, whole-system flag threaded down from
`init = system_state < SYSTEM_RUNNING` inside
[`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886)
itself ([§8.5](#jump-label-update-jump-label-update)), true a little
...
### 9.2 Batching API used by jump labels {#batching-api-used-by-jump-labels}

[§9.1](#early-boot-vs-live-smp) settled *how* a single site gets patched
once the decision to patch it is made; this section is about *when* jump
labels actually pull that trigger. A busy tracepoint can have thousands
of call sites sharing one key, and paying the full IPI-synchronized
protocol ([§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish))
separately for each one would be needless — the sites can be collected
first and the expensive part paid once for the whole group. Two
arch-level hooks make that possible: one that queues the newly computed
bytes for a site without touching hardware yet, and one that flushes
everything queued so far in a single synchronized pass:
...
one
[`batch_finish()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2941)
...
[`smp_text_poke_batch_add()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L3202)
flushes early
([`text_poke_addr_ordered()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L3170)
in
[`alternative.c`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c)).
That is why
[`jump_label_cmp`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L35)
...
[`struct smp_text_poke_loc`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2781-L2792):
...
[`_stext`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/vmlinux.lds.S#L137)[^3],
...
[§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish)) to fully
enable/disable.

[`__jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L503)
([§8.5](#jump-label-update-jump-label-update)) does contain a generic
...
[`smp_text_poke_loc`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2781-L2792)
...
[§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish).
...
### 9.3 The INT3 SMP algorithm ([`smp_text_poke_batch_finish`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2941)) {#the-int3-smp-algorithm-smp-text-poke-batch-finish}

This is the payoff of everything
[§4.3](#why-you-cannot-just-memcpy-over-live-code-on-smp) through
[§9.2](#batching-api-used-by-jump-labels) built toward. The key fact
from [§4.3](#why-you-cannot-just-memcpy-over-live-code-on-smp) was that
...
[`cond_resched()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/sched.h#L2161)
...
[`smp_text_poke_int3_handler()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2838)
...
final byte, and
[`smp_text_poke_sync_each_cpu()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2771)
...
> [`traps.c`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/traps.c).
...
> [`atomic_set_release()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/atomic/atomic-instrumented.h#L83),
> then
> [`smp_wmb()`](https://elixir.bootlin.com/linux/v7.2/source/include/asm-generic/barrier.h#L107)s,
...
> [`text_poke_array`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2797)
...
>   [`int3_emulate_jmp()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/text-patching.h#L136)
...
### 9.4 What “sync” means {#what-sync-means}

The three-phase protocol from
[§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish) names “IPI
...
does ([§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish) already
covered that), but what every *other* CPU is forced to do in response.
...
[§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish) depends on
...
[§4.1](#what-a-cpu-actually-does-with-instructions) finally pays for
...
[`X86_FEATURE_SERIALIZE`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/cpufeatures.h#L431),
...
[`iret_to_self()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/sync_core.h#L22).
...
[`iret_to_self()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/sync_core.h#L22)
does the half of that job the CPU would do: it pushes those same five
words by hand, with no real interrupt behind them, sets the saved `RIP`
field to the very next instruction after the `iret`, and then executes
`iret` against that fabricated frame. As far as the CPU can tell, this
is a completely ordinary return from an interrupt, so it does everything
...
[`arch/x86/include/asm/sync_core.h`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/sync_core.h#L22)
...
### 9.5 Writing through RO mappings {#writing-through-ro-mappings}

Every store [§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish)
...
[§4.4](#writing-read-only-kernel-text-text-poke) dance in full: build
...
[`text_mutex`](https://elixir.bootlin.com/linux/v7.2/source/kernel/extable.c#L27),
and this is the piece [§4.4](#writing-read-only-kernel-text-text-poke)
...
path ([§9.2](#batching-api-used-by-jump-labels)),
[`text_mutex`](https://elixir.bootlin.com/linux/v7.2/source/kernel/extable.c#L27)
...
[`smp_text_poke_batch_finish()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2941)
from [§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish) — that
...
### 9.6 End-to-end timeline for one enable {#end-to-end-timeline-for-one-enable}

[§9.1](#early-boot-vs-live-smp) through
[§9.5](#writing-through-ro-mappings) examined the machinery one piece at
a time — which patch path early boot takes, how sites get batched, the
three INT3 phases, what “synchronize” actually does on the wire, and how
each of those writes reaches memory that is nominally read-only. Laid
end to end, a single
[`static_branch_enable()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L522)
...
per site ([§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish)).
The only thing that grows that fixed cost is the
[§9.2](#batching-api-used-by-jump-labels) overflow case: past 256 queued
sites, a second full
[`batch_finish()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2941)
...
## 10 Modules: the trickiest part {#modules-the-trickiest-part}
...
modules. The picture from [§6.1](#struct-static-key) of
[`key->entries`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L97)
...
the way [§8.1](#boot-jump-label-init) settles it for the `vmlinux`-only
...
word/pointer union from [§6.1](#struct-static-key) can end up in exactly
three states:
...
- **Linked:** a second object has registered call sites for the same
  ...
  [`key->next`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L98)
...
- **Sealed:** a key living in
  ...
  [`jump_label_init_ro()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L573)
  runs ([§8.1](#boot-jump-label-init) covers the mechanics and the bit
...
[`static_key_mod`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L613)
...
[`jump_label_del_module()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L762)
when one unloads. Neither is called directly — both run from a module
notifier[^5]
([`jump_label_module_notify()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L817),
registered with
[`.priority = 1`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L845))
that hooks
[`MODULE_STATE_COMING`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/module.h#L308)/
[`MODULE_STATE_GOING`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/module.h#L309)
...
[`.priority = 0`](https://elixir.bootlin.com/linux/v7.2/source/kernel/tracepoint.c#L688)
...
### 10.1 Loading: [`jump_label_add_module()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L690) {#loading-jump-label-add-module}
...
[§8.1](#boot-jump-label-init) already covered, and its `-ENOMEM` error
...
[§8.1](#boot-jump-label-init) used on the `vmlinux`-wide table, for the
...
[§10](#modules-the-trickiest-part) corresponds to exactly one path
...
[`within_module()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/module.h#L657)
asks whether the
[`static_key`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L86)
...
[`enabled`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L87)
and
[`entries`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L97)
...
before building any list node the **sealed** case has to be checked:
...
([§8.1](#boot-jump-label-init)), so there is nothing left to link this
...
[§10](#modules-the-trickiest-part) actually gets built or grown, in two
steps.
...
[`__module_address()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/module/main.c#L3887),
and installs `jlm2` as the head of a new one-node list. Once
[`static_key_set_linked()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L430)
...
[`key->entries`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L97).
[`static_key_set_mod()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L631)
makes `jlm` the new head of the list, with the previous head linked
behind it via `jlm->next`.

Both the sealed-key
[`goto`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L727)
...
[§5.1](#two-polarities-key-default-branch-hint), exposed here as
[`jump_label_init_type()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L603)
— which decided whether the assembler macro emitted this site as a `nop`
or a `jmp` in the first place ([§5.2](#how-the-macros-pick-the-asm)),
...
[`static_branch_enable()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L522)/`disable()`
on this key before the module ever loaded.
[`do_poke`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L754)
...
### 10.2 Toggling an already-linked key {#toggling-an-already-linked-key}
...
from [§8.5](#jump-label-update-jump-label-update) is not enough — the
...
the flat walk from [§8.5](#jump-label-update-jump-label-update). Each
module has its own private
[`__jump_table`](https://elixir.bootlin.com/linux/v7.2/source/scripts/module.lds.S#L31)
section ([§6.4](#linker-section)), bounded by the
...
Second, `m` — the
[`mod`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L616)
...
`NULL` originates in [§10.1](#loading-jump-label-add-module): the
...
[`__stop___jump_table`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L210),
which marks the end of the vmlinux-wide table.

Third,
[`mod->entries`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L615)
can genuinely be `NULL`. That happens when an object defines a key but
contains no call sites for it — the linking code from
[§10.1](#loading-jump-label-add-module) still creates a list node to
...
[`MODULE_STATE_LIVE`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/module.h#L307)
has discarded that section — patching into freed memory would be a
use-after-free, not a harmless no-op.

### 10.3 Unloading: [`jump_label_del_module()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L762) {#unloading-jump-label-del-module}
...
the
[`static_key_mod`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L613)
node that [§10.1](#loading-jump-label-add-module) created. The
...
[§10.1](#loading-jump-label-add-module) directly:
...
[§10.1](#loading-jump-label-add-module): skip duplicate entries for the
...
[`WARN_ON`](https://elixir.bootlin.com/linux/v7.2/source/include/asm-generic/bug.h#L118)
...
[§10.1](#loading-jump-label-add-module) running in reverse: a key that
...
## 11 Fallback: [`CONFIG_JUMP_LABEL=n`](https://elixir.bootlin.com/linux/v7.2/source/arch/Kconfig#L130) {#fallback-config-jump-label-n}

[`JUMP_LABEL`](https://elixir.bootlin.com/linux/v7.2/source/arch/Kconfig#L130)
is optional. Most distro kernels end up with it on — arm64 selects it
outright, and on x86
[`PREEMPT_DYNAMIC`](https://elixir.bootlin.com/linux/v7.2/source/kernel/Kconfig.preempt#L129)
pulls it in — but a minimal config can legitimately leave it off.
Everything from [§4](#hardware-background-why-this-is-hard) onward
...
`entries`/`next`/`type` union from [§6](#core-data-structures) is
...
[`static_branch_likely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L500)
and
[`static_branch_unlikely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L501)
reduce to:
...
[`static_key_count()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L246)
is a plain
...
[§8.2](#enabling-static-key-enable-static-branch-enable)). Without
patching, `enabled` never goes negative, so the clamp is unnecessary.

[`static_key_enable()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L307)
and
...
## 12 Worked micro-example (bytes on the wire) {#worked-micro-example-bytes-on-the-wire}
...
1.  `f()` is called after boot, on a live, multi-CPU system — the
    interesting [§9](#x86-text-patching-the-gory-details) INT3 path, not
    the single-CPU [§9.1](#early-boot-vs-live-smp) boot shortcut (a
...
2.  The compiler places `l_yes` — the out-of-line block containing the
    ...
    [§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes) picks the
    ...
    below ([§4.2](#x86-instruction-encoding-jmp-and-nop)); nothing else
    about the trace would change.

### 12.1 Compile / link / `objtool` ([`HAVE_JUMP_LABEL_HACK`](https://elixir.bootlin.com/linux/v7.2/source/arch/Kconfig#L1399)) {#compile-link-objtool-have-jump-label-hack}
...
1.  `k` is a `FALSE` key read with
    [`static_branch_unlikely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L486).
    Per the table in [§5.2](#how-the-macros-pick-the-asm), that
    combination calls the nop-default helper
    [`arch_static_branch(&k.key, false)`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L35),
    not the jmp-default
    [`arch_static_branch_jump()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L45).
    That is the `type ^ branch` formula from
    [§5.1](#two-polarities-key-default-branch-hint) at work: `type = 0`
...
2.  The `asm goto` inside
    ...
    via
    [`JUMP_TABLE_ENTRY()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/jump_label.h#L15)
    ([§5.4](#the-jump-table-entry-sidecar-metadata)): `code` is the
    ...
    [`unlikely()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/compiler.h#L77)
    hint. Bit 1 is the build-time signal to `objtool`: “NOP this jmp”
    ([§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes)); after
    boot,
    [`jump_entry_set_init()`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L163)
...
3.  The assembler picks the encoding by the real distance to `l_yes` —
    ...
    from [§4.2](#x86-instruction-encoding-jmp-and-nop)). The two bytes
    actually sitting at `1:` right after assembly, before `objtool` ever
    runs, are `EB 50`.

4.  During the build, `objtool` calls
    [`handle_jump_alt()`](https://elixir.bootlin.com/linux/v7.2/source/tools/objtool/check.c#L1872)
    ([§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes) step 3),
    ...
    [§4.2](#x86-instruction-encoding-jmp-and-nop)) — same size, so
...
5.  At boot,
    ...
    sorts
    [`__jump_table`](https://elixir.bootlin.com/linux/v7.2/source/include/asm-generic/vmlinux.lds.h#L436)
    by key, then loops over every entry ([§8.1](#boot-jump-label-init)).
...
6.  The same loop iteration does two pieces of bookkeeping that wire `k`
    ...
    [`static_key_set_entries()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L444)
    points
    [`k.entries`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L97)
    at this entry — the pointer that
    [`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886)
    will follow when
    [`static_branch_enable(&k)`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L522)
    runs later. With that in place, the hot path in `f()` from the very
    first time it runs is: decode `66 90` (falls through, no load of
    `k`, [§4.1](#what-a-cpu-actually-does-with-instructions)), then
    `call something`.
...
### 12.2 Runtime [`static_branch_enable(&k)`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L522) {#runtime-static-branch-enable-k}

[§9.6](#end-to-end-timeline-for-one-enable) already lays out the full
...
1.  [`static_branch_enable(&k)`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L522)
    expands to
    [`static_key_enable(&k.key)`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L220),
    which acquires
    [`jump_label_lock()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L25)
    and walks the `0 → -1 → (patch) → 1` state machine from
    [§8.2](#enabling-static-key-enable-static-branch-enable).
    [`enabled`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L87)
    ...
    [`jump_label_update(&k.key)`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886)
    does the actual patching, and only after it returns does
    [`enabled`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L87)
    get published as `1` with release ordering.
2.  [`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886)
...
3.  [`arch_jump_label_transform_queue()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/jump_label.c#L123)
    calls
    [`__jump_label_patch()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/jump_label.c#L36),
    which re-derives the size by decoding the live bytes at `1:` —
    [`arch_jump_entry_size()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/jump_label.c#L20)
    returns 2, matching what `objtool` left behind
    ([§7](#size-of-the-patchable-site-on-x86-runtime)). The function
    then builds both sequences: `nop = 66 90` (from `x86_nops[2]`) and
    `code = EB 50` (from
    [`text_gen_insn()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/include/asm/text-patching.h#L123)
    — the same bytes computed in
    [§12.1](#compile-link-objtool-have-jump-label-hack) step 3, since
    ...
    [`smp_text_poke_batch_add()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L3202)
    ([§9.2](#batching-api-used-by-jump-labels)).
4.  The loop of
    [`jump_label_update()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L886)
    ...
    [§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish) on this
...
5.  From the instant the sync in phase 1 completes, any CPU landing on
    ...
    ([§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish)); phases
    2-3 only retire the trap-and-emulate path in favor of the real
    bytes. Once phase 3 lands, every subsequent call to `f()` decodes
    `EB 50`, jumps 80 bytes forward into `l_yes`, runs `printk("on\n")`,
    then hits the compiler-emitted `jmp back`
    ([§2](#the-mental-model-in-one-diagram)) and falls into
    `something()`.

### 12.3 [`static_branch_disable(&k)`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L523): same protocol, asymmetric math {#static-branch-disable-k-same-protocol-asymmetric-math}
...
→
[`static_key_disable(&k.key)`](https://elixir.bootlin.com/linux/v7.2/source/kernel/jump_label.c#L245)
— a single
...
state to protect ([§8.3](#disabling): a reader who sees stale “on” for a
...
([§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish)) runs again,
in the opposite byte direction:
...
lifecycle that
[§4](#hardware-background-why-this-is-hard)-[§9](#x86-text-patching-the-gory-details)
...
[§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish) every time
after that.

| Stage | Live bytes at `1:` | Who wrote them |
|----|----|----|
| After assembly, before `objtool` | `EB 50` | Compiler/assembler ([§5.3](#the-two-asm-helpers), [§4.2](#x86-instruction-encoding-jmp-and-nop)) |
| After `objtool`, at boot | `66 90` | [`handle_jump_alt()`](https://elixir.bootlin.com/linux/v7.2/source/tools/objtool/check.c#L1872) ([§5.5](#have-jump-label-hack-why-sites-are-2-or-5-bytes)) |
| After `static_branch_enable(&k)` | `EB 50` | [`__jump_label_patch()`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/jump_label.c#L36) via INT3 protocol ([§9.3](#the-int3-smp-algorithm-smp-text-poke-batch-finish)) |
| After `static_branch_disable(&k)` | `66 90` | Same, reverse direction |

------------------------------------------------------------------------

## 13 Further reading in-tree {#further-reading-in-tree}

- [Jump label](https://lwn.net/Articles/412072/) (LWN, 2010) — the
  original write-up of the idea.
- [`Documentation/staging/static-keys.rst`](https://elixir.bootlin.com/linux/v7.2/source/Documentation/staging/static-keys.rst#L2)
  — historical overview and old `getppid` instruction dump (still
  useful; ignore fixed “always 5 bytes” claims for modern x86).
- [`arch/x86/kernel/alternative.c`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2941)
  —
  [`smp_text_poke_batch_finish`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2941)
  comment block (canonical INT3 algorithm write-up).
- [`tools/objtool/check.c`](https://elixir.bootlin.com/linux/v7.2/source/tools/objtool/check.c#L1872)
  —
  [`handle_jump_alt()`](https://elixir.bootlin.com/linux/v7.2/source/tools/objtool/check.c#L1872)
  for the jmp→nop hack.
- [`include/linux/jump_label.h`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/jump_label.h#L31)
  — the top comment in the header: deprecated vs. current API,
  behavioral model, the “absolute slow paths” warning,
  deferred-decrement rationale.
- [`include/linux/tracepoint-defs.h`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/tracepoint-defs.h#L39)
  —
  [`struct tracepoint`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/tracepoint-defs.h#L39)
  embeds both a
  [`static_key_false`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/tracepoint-defs.h#L41)
  and a
  [`static_call_key`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/tracepoint-defs.h#L42)
...
- Sister mechanisms with the same poke engine: [**static
  calls**](https://elixir.bootlin.com/linux/v7.2/source/include/linux/static_call.h),
  [**ftrace**](https://elixir.bootlin.com/linux/v7.2/source/Documentation/trace/ftrace-design.rst),
  [**kprobes**](https://elixir.bootlin.com/linux/v7.2/source/Documentation/trace/kprobes.rst)
  — same
  [`text_poke`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/alternative.c#L2668)
...
[^3]: `_stext` is a linker-defined symbol, not a C variable — it marks
    ...
    directly in
    [`arch/x86/kernel/vmlinux.lds.S`](https://elixir.bootlin.com/linux/v7.2/source/arch/x86/kernel/vmlinux.lds.S#L137).
    ...
    ([§5.4](#the-jump-table-entry-sidecar-metadata)) for its
...
[^5]: A module notifier is a callback registered against the module-load
    notifier chain
    ([`module_notify_list`](https://elixir.bootlin.com/linux/v7.2/source/kernel/module/main.c#L164))
    via
    ...
    [`blocking_notifier_call_chain()`](https://elixir.bootlin.com/linux/v7.2/source/kernel/notifier.c#L368)
    at each state transition
    ([`MODULE_STATE_COMING`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/module.h#L308),
    [`MODULE_STATE_GOING`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/module.h#L309),
    etc.), invoking every registered
    [`struct notifier_block`](https://elixir.bootlin.com/linux/v7.2/source/include/linux/notifier.h#L54)
